...
y_imp_train = np.loadtxt('data/Y_train_classification.txt').tolist()
...
y_imp_test = np.loadtxt('data/Y_test_classification.txt').tolist()

def stats(X_train, y_train, X_dev, y_dev, X_test, y_test, model, label):
	print("*="*10, "  ", label, "  ", "=*"*20)
	print("**** Classification reports for train, dev, test: ")
	print(classification_report(y_train, model.predict(X_train)))
	print(classification_report(y_dev, model.predict(X_dev)))
	print(classification_report(y_test, model.predict(X_test)))

	print("**** Accuracies for train, dev, test: ")
	print(accuracy_score(y_train, model.predict(X_train)))
	print(accuracy_score(y_dev, model.predict(X_dev)))
	print(accuracy_score(y_test, model.predict(X_test)))
	print("\n\n")


print('Naive classifier performance:')
guess = [mode(y_imp_train)[0][0]]
print(accuracy_score(y_imp_train, guess * len(y_imp_train)))
print(accuracy_score(y_imp_dev, guess * len(y_imp_dev)))
print(accuracy_score(y_imp_test, guess * len(y_imp_test)))
...
stats(X_imp_train, y_imp_train, X_imp_dev, y_imp_dev, X_imp_test, y_imp_test, svc, "SVMC")

# Random Forest Classifier
rf = RandomForestClassifier()
rf.fit(X_imp_train, y_imp_train)
dev_predictions_rf = rf.predict(X_imp_dev)
stats(X_imp_train, y_imp_train, X_imp_dev, y_imp_dev, X_imp_test, y_imp_test, rf, "Random Forest")
...
for h in hSizes:
	...
	stats(X_imp_train, y_imp_train, X_imp_dev, y_imp_dev, X_imp_test, y_imp_test, nn_model, "NN with layers {}".format(h))
